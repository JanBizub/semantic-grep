...
Tests cover the pure/mockable units — `MarkdownChunker`, `PageMap`, `MarkdownDocumentParser`, `InterpreterService` and `FigureCaptioner` (via a fake `IChatClient`), `FigureCaptionInjector`, `ContextFormatter`, `TermOccurrenceFormatter`, RRF fusion + per-document capping in `HybridSearch`, the `TermSearch` regex/overlap helpers, and `SelfUpdater` checksum/tag parsing. A few pure static helpers are `internal` (exposed via `InternalsVisibleTo` to `Segrep.Tests`) rather than `private` for this reason. DB-bound code (`Store/`, the SQL search legs) has no coverage; that needs integration tests against a real Postgres (tracked as `semantic-grep-ff9`).

## CLI commands

Registered in `Segrep/Program.cs`; implemented under `Segrep/Commands/`:

- `index [<path>] [--force] [--describe-images]` — ingest a folder or file: parse → chunk → embed → store. Skips files whose content hash is unchanged unless `--force`. Defaults to the current directory. `--describe-images` additionally captions each detected figure with the vision model (one LLM call per figure — costs money and slows ingestion) and injects the descriptions into the chunked text so purely visual content becomes searchable.
...
**Image understanding** (`index --describe-images`): every analyze call also requests `AnalyzeOutputOption.Figures` and downloads the service-cropped figure PNGs into `{hash}.figures/` plus a `{hash}.figures.json` manifest (figure id, Markdown span, page) — cropped images are only downloadable for a short window after analysis, so they are always cached even when captioning isn't requested. With `--describe-images`, `DocumentIntelligence/FigureCaptioner` sends each cached figure PNG to a vision `IChatClient` (keyed DI registration `vision-chat`, deployment `AzureOpenAI:VisionDeploymentName` falling back to `ChatDeploymentName`), skipping figures under 40 px in either dimension; successful captions are cached in `{hash}.captions.json` (failures warn, aren't persisted, and retry next run). `FigureCaptionInjector` then inserts `**Image description:** …` before each figure's `</figure>` in-memory and shifts the `PageMap` spans so page attribution stays correct — the enriched Markdown feeds the normal chunk → embed → store path. The cached `{hash}.md` stays raw DI output, so `find`/`TermSearch` exact counts deliberately exclude synthetic captions. A standalone image file in which DI found no figures is captioned whole and the description appended. Cache entries created before this feature lack the figures manifest and are re-analyzed only when `--describe-images` needs them (plain `index` keeps using them).
...
- `AzureOpenAI` — chat/interpreter model endpoint + key; optional `VisionDeploymentName` for figure captioning (blank = use `ChatDeploymentName`).
