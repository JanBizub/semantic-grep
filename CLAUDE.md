...
**Retrieval** (`enrich`/`ask`): `Search/HybridSearch` fans out to three legs in parallel — `SemanticSearch` (pgvector HNSW cosine ANN), `FullTextSearch` (`tsvector`/`ts_rank`), and `GrepSearch` (`pg_trgm` `similarity` + `ILIKE`) — then fuses the ranked lists with **Reciprocal Rank Fusion** (RRF, constant `K = 60`) and takes the top-K. `enrich` stops after augmentation; `ask` calls `InterpreterModel/InterpreterService` (an `IChatClient`) to compose a cited answer. `InterpreterService` also provides query expansion used by both commands unless `--raw`.
